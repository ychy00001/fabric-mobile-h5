...
import { AtButton, AtCurtain , AtToast } from "taro-ui";
...
const Index = () => {
  ...
  const [toastText, setToastText] = useState<string>("数据异常");
  const [currentTmpId, setCurrentTmpId] = useState<string>("");
  ...
  useEffect(() => {
    ...
    if(router){
      const tmpId : string = router.params['tmpId'] ? router.params['tmpId'] : ""
      setCurrentTmpId(tmpId)
      renderJson(tmpId)
    }else{
      setOpenToast(true)
      setToastText("模版ID不存在")
    }
  }, []);
  ...
  const handleSaveTpl = () => {
    ...
    tplImgs[id] = imgUrls;
    localStorage.removeItem("tplImgs")
    ...
    <>
      <View className="index-index">
        <View className="head-top-view">
          <View className="head-row-item-l">
            <AtButton
      ...
      <AtToast isOpened={openToast} text={toastText} status="error"></AtToast>
    </>
  );
};
